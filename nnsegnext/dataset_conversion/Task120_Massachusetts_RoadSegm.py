...
from nnsegnext.utilities.file_conversions import convert_2d_image_to_nifti
...
if __name__ == '__main__':
    ...
    `nnSegnext_plan_and_preprocess` command like this:
    
    > nnSegnext_plan_and_preprocess -t 120 -pl3d None
    
    once that is completed, you can run the trainings as follows:
    > nnSegnext_train 2d nnSegnextTrainerV2 120 FOLD
    
    (where fold is again 0, 1, 2, 3 and 4 - 5-fold cross validation)
    
    there is no need to run nnSegnext_find_best_configuration because there is only one model to shoose from.
    Note that without running nnSegnext_find_best_configuration, nnU-Net will not have determined a postprocessing
    ...
    `nnSegnext_determine_postprocessing` command
    """
