...
from nnsegnext.evaluation.add_mean_dice_to_json import foreground_mean
from nnsegnext.evaluation.evaluator import evaluate_folder
from nnsegnext.evaluation.model_selection.ensemble import ensemble
from nnsegnext.paths import network_training_output_dir
import numpy as np
from subprocess import call
from nnsegnext.postprocessing.consolidate_postprocessing import consolidate_folds, collect_cv_niftis
from nnsegnext.utilities.folder_names import get_output_folder_name
from nnsegnext.paths import default_cascade_trainer, default_trainer, default_plans_identifier
...
def main():
    ...
    parser.add_argument("-tr", type=str, required=False, default=default_trainer,
                        help="nnSegnextTrainer class. Default: %s" % default_trainer)
    parser.add_argument("-ctr", type=str, required=False, default=default_cascade_trainer,
                        help="nnSegnextTrainer class for cascade model. Default: %s" % default_cascade_trainer)
        ...
        for k, v in results.items():
            if v == best:
                ...
                if k.startswith("ensemble"):
                    ...
                    predict_str += "nnSegnext_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr " + tr + " -ctr " + trc + " -m " + m1 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"
                    predict_str += "nnSegnext_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL2 -tr " + tr + " -ctr " + trc + " -m " + m2 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"

                    if not disable_postprocessing:
                        predict_str += "nnSegnext_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER -pp " + join(network_training_output_dir, "ensembles", id_task_mapping[t], k, "postprocessing.json") + "\n"
                    else:
                        predict_str += "nnSegnext_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER\n"
                else:
                    predict_str += "nnSegnext_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr " + tr + " -ctr " + trc + " -m " + k + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"
                print(predict_str)
