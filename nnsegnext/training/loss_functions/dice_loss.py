...
import torch
from nnsegnext.training.loss_functions.TopK_loss import TopKLoss
from nnsegnext.training.loss_functions.crossentropy import RobustCrossEntropyLoss
...
def entropy_map(prob):
    ...
    entropy = torch.softmax(entropy, dim=2)
    return entropy


def EntropyDiceloss(outputs, gt, weight):
    ...
    gt_n = gt_n[:, 1:, :, :]
    gt_n = gt_n.contiguous().view(batch_size,num_classes, -1)
    
    prob_n = torch.softmax(outputs, dim=1)
    prob_n = prob_n[:, 1:, :, :]   
    prob_n = prob_n.contiguous().view(batch_size,num_classes, -1)
       
    intersection = prob_n * gt_n
    
    loss = (2 * (intersection).sum(2) + smooth) / ((prob_n).sum(2) + (gt_n).sum(2) + smooth)
    loss = 1 - loss.sum(1) /num_classes
    loss = (loss * weight).sum()
    return loss 
  

class DC_and_CE_wED_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
                 log_dice=False, ignore_label=None):
        """
        CAREFUL. Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        ...
        :param weight_dice:
        """
        super(DC_and_CE_wED_loss, self).__init__()
        if ignore_label is not None:
            assert not square_dice, 'not implemented'
            ce_kwargs['reduction'] = 'none'
        self.log_dice = log_dice
    ...
    def forward(self, net_output, target,w):
        ...
        ed_loss = EntropyDiceloss(net_output, target.long(), w)
        
        if self.aggregate == "sum":
            result = self.weight_ce * ce_loss + self.weight_dice * dc_loss + self.weight_dice * ed_loss
        else:
            raise NotImplementedError("nah son") # reserved for other stuff (later)
        return result
