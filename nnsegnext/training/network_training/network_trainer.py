...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        if 'nnsegnext_use_progress_bar' in os.environ.keys():
            self.use_progress_bar = bool(int(os.environ['nnsegnext_use_progress_bar']))
    ...
    def plot_network_architecture(self):
        """
        can be implemented (see nnSegnextTrainer) but does not have to. Not implemented here because it imposes stronger
        ...
        if self.patience is not None:
            ...
            if self.val_eval_criterion_MA > self.best_val_eval_criterion_MA:
                ...
                if self.save_best_checkpoint: 
                    ...
                    if self.best_val_eval_criterion_MA >= self.target: 
                        self.save_checkpoint(join(self.output_folder, f"model_{self.val_eval_criterion_MA}.model"))
                        self.target += 0.002
                        self.print_to_log_file("save checkpoint done!", self.val_eval_criterion_MA)
