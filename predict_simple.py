...
import glob
import re
from nnsegnext.inference.predict import predict_from_folder
from nnsegnext.paths import default_plans_identifier, network_training_output_dir, default_cascade_trainer, default_trainer
...
def main():
    ...
    parser.add_argument("-i", '--input_folder',default='/home/xuesheng3/liuyuchen/Data/nnSegnext_raw_data_base/nnSegnext_raw_data/Task004_BrainTissueIBSR/imagesTs', help="Must contain all modalities for each patient in the correct"
                                                     " order (same as training). Files must be named "
                                                     "CASENAME_XXXX.nii.gz where XXXX is the modality "
                                                     "identifier (0000, 0001, etc)")
    parser.add_argument('-o', "--output_folder",default='/home/xuesheng3/liuyuchen/Data/nnSegnext_raw_data_base/Infer/IBSR/IBSR_IBSR', help="folder for saving predictions")
    parser.add_argument('-t', '--task_name', help='task name or task ID, required.',default='Task004_BrainTissueIBSR')
    ...
    parser.add_argument('-tr', '--trainer_class_name',
                        help='Name of the nnFormerTrainer used for 2D U-Net, full resolution 3D U-Net and low resolution '
                             ...
                             % default_trainer,
                        required=False,
                        default='nnSegnextTrainerV2_segnext')
    ...
    modellist = glob.glob(model_folder_name+'/fold_0/model_*.model')
    modelname = list_sort_nicely([model.split('/')[-1].split('.m')[0] for model in modellist])[:-2]
    print(modelname)
    for name in modelname:
        print('===========================================')
        print(f'Model {name} predict begin!!!')
        print('===========================================')
        output_fold = output_folder + '/' + name
        predict_from_folder(model_folder_name, input_folder, output_fold, folds, save_npz, num_threads_preprocessing,
                            num_threads_nifti_save, lowres_segmentations, part_id, num_parts, not disable_tta,
                            overwrite_existing=overwrite_existing, mode=mode, overwrite_all_in_gpu=all_in_gpu,
                            mixed_precision=not args.disable_mixed_precision,
                            step_size=step_size, checkpoint_name=name)
        print('===========================================')
        print(f'Model {name} predict done!!!')
        print('===========================================')
